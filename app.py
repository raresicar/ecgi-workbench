"""StabFEM ECGi Workbench — interactive forward/inverse cardiac electrophysiology.

Runs live on the workstation; VS Code forwards the port to laptop browser:
...
Workflow:
  ...
  4. Inspect truth vs recovered per frame, the localisation error, and play the
     frames as an animation so the potentials' evolution is visible.
...
st.set_page_config(page_title="stabFEM ECGi Workbench", page_icon="🫀", layout="wide")
W = "stretch"

# these do not reload meshes every rerun -> C.get_* is @st.cache_resource
geo = C.get_geometry()
...
# ----------------------------------------------------------------------------
# Sidebar: tissue (selects database) + inverse / simulation settings
# ----------------------------------------------------------------------------
st.sidebar.title("🫀 StabFEM ECGi Workbench")
st.sidebar.caption("Live forward & inverse cardiac electrophysiology simulator.")

# choice of tissue type (healthy/infarct) also selects the database used for POD
...
# ----------------------------------------------------------------------------
# options: forward or inverse run -> inverse needs first run
...
with lab_tab:
    ...
    with right:
        ...
        if st.button("▶ Simulate & reconstruct", type="primary", width=W):
            ...
            # forward run to compute measurements -> first half of progress bar
            ...
            # inverse run for each frame -> second half of progress bar
        ...
        if series:
            # display best results over all frames
    ...
    # ---- results plot: single-frame or animation -------------------------------
    series = st.session_state.get("series")
    if series:
        ...
        st.divider()
        # options to view results as single frame or animation
        ...
        if view == "Single frame":
            ...
            g1, g2 = st.columns(2)
            # use plotly for plotting the meshes
